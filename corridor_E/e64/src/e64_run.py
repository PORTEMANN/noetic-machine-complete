...
DT = 0.25
KAPPAS = [0.025, 0.05, 0.1, 0.2]        # figé ; 0,05 = contrôle E61/E62
R_T0 = 10.0
TOL_T0 = 0.30
TOL_T0_SCALING = 0.15
TMAX = 180.0
...
def main():
    ...
    t0 = time.time()
    res = {"campagne": "E64", "protocole": "E64-FENETRE-KAPPA-1.0 (gelé)",
           "points": {}}
...
def _ecris(res, t0):
    ...
    out = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                       "..", "data", "e64_fenetre_kappa_verdict.json")
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, "w", encoding="utf-8") as f:
