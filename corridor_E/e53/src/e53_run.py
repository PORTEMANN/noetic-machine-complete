...
# -*- coding: utf-8 -*-
"""
E53 — GUIDES TORIQUES : un canal continu de rayon fixe conserve-t-il le
lien imprimé contre la tension de ligne ?   [protocole E53-GUIDES-1.0 gelé]
=====================================================================
Harnais officiel de la campagne E53. Protocole gelé dans
e53_protocole.json (haché avant tout calcul — aveugle préservé).

Filiation déclarée : détecteur E44 byte-identique (e44_core.py,
sha256 604c2232…60ae1ac7) ; construction du lien imprimé d'E50
(identique) ; suivi spatial d'E48→E51 (inchangé).

Verdicts moteurs (déclarés) : E51 (B3-FAIL lisible) — des puits ponctuels
sur le cercle initial du cœur n'empêchent pas la contraction radiale de
l'anneau (tension de ligne) : l'anneau quitte les puits en se
contractant ; E52 (PARTIEL) — les puits ponctuels écartent la nucléation
au lieu d'ancrer. Réparation désignée par les données : une structure
ÉTENDUE continue — le canal torique (précédent physique : courants
persistants en canal torique), qui oppose une force de rappel continue à
la contraction.

Construction gelée :
  Lien : identique à E50/E51 (anneau 1 : plan xy, centre
  (31,5;31,5;31,5), R=10 ; anneau 2 : plan xz, centre (41,5;31,5;31,5),
  r=8 ; δ=1,5 ; relaxation 100 pas à γ=0,3 SANS guide).
  Guide (bras B) : V_ext(x) = −U_g·exp(−d1²/2σ²) − U_g·exp(−d2²/2σ²),
  où d1, d2 sont les distances AU CERCLE de chaque anneau (canal torique
  continu) ; U_g = 2, σ = 1,5 (figés, filiation E51).
  Bras C (spécificité) : canaux décalés de +3 mailles en rayon sur
  chaque anneau (figé).
Bras depuis l'état relaxé (t=5) : A : γ=0,3 sans guide (témoin —
reproduit E50-A/E51-A) ; B : γ=0,3 + canaux appariés ; C : γ=0,3 +
canaux décalés. Snapshots : t=5, 45, 90. Suivi : identique à E48→E51.
Campagne déterministe — test d'existence, déclaré.

Usage : python3 e53_run.py            (campagne — interdit avant décision)
        python3 e53_run.py --smoke    (validation technique déclarée :
...
R_ANNEAU2 = 8.0
DELTA_COEUR = 1.5
U_GUIDE = 2.0          # profondeur des canaux, figée (filiation E51)
SIGMA_GUIDE = 1.5      # largeur radiale des canaux, figée
DECALAGE_C = 3.0       # bras C : décalage radial, figé
...
def geometrie_lien(N_=N, decal=0.0):
    """Anneaux gelés (identique E50) + distances aux cercles (canaux).
    decal : décalage radial des canaux (bras C), figé."""
    C = (N_ - 1) / 2.0
    s = N_ / 64.0
    R1, R2 = R_ANNEAU1 * s, R_ANNEAU2 * s
    R1g, R2g = R1 + decal * s, R2 + decal * s
    ...
    psi = (np.tanh(d1 / (DELTA_COEUR * s)) * np.tanh(d2 / (DELTA_COEUR * s))
           * np.exp(1j * (phi1 + phi2)))
    # distances aux cercles des canaux (décalés si decal > 0)
    u1g, w1g = rho1 - R1g, Z - C
    d1g = np.sqrt(u1g**2 + w1g**2)
    u2g, w2g = rho2 - R2g, Y - C
    d2g = np.sqrt(u2g**2 + w2g**2)
    return psi, d1g, d2g, s


def V_guides(d1g, d2g, N_=N, s=1.0):
    sig = SIGMA_GUIDE * s
    return -U_GUIDE * (np.exp(-d1g**2 / (2 * sig**2))
                       + np.exp(-d2g**2 / (2 * sig**2)))
...
def main():
    if "--smoke" in sys.argv:
        ...
        psi, d1g, d2g, s = geometrie_lien(32)
        Vb = V_guides(d1g, d2g, 32, s)
        psi2, d1c, d2c, _ = geometrie_lien(32, decal=DECALAGE_C)
        Vc = V_guides(d1c, d2c, 32, s)
        lin = gp_linear(32, A, GAMMA, DT)
        for _ in range(20):
            psi = gp_step(psi, lin, GAMMA, DT)
        det = detecte(psi, avec_loops=True)
        print("SMOKE :", {"nboucles": det["nboucles"], "npaires": det["npaires"],
                          "paires": det["paires"], "anom": det["anom"],
                          "amb": det["amb"],
                          "V_canal_min": round(float(Vb.min()), 3),
                          "V_decal_min": round(float(Vc.min()), 3)})
        return

    t0 = time.time()
    res = {"campagne": "E53", "protocole": "E53-GUIDES-1.0 (gelé)"}

    # ---- construction + relaxation (sans guide — identique E50/E51) ----
    psi, d1g, d2g, s = geometrie_lien(N)
    _, d1c, d2c, _ = geometrie_lien(N, decal=DECALAGE_C)
    ...
    # ---- trois bras ----
    Vb = V_guides(d1g, d2g, N, s)
    Vc = V_guides(d1c, d2c, N, s)
    ...
    # ---- agrégation gelée (règles : e53_protocole.json) ----
    def pers(b):
